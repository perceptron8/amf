...
AMF3.Writer.prototype.writeInteger = function(number) {
	if (number < 0x80) {
...
AMF3.Writer.prototype.writeString = function(string) {
	if (!_.isEmpty(string)) {
		if (!this.strings.has(string)) {
			...
			this.writeInteger(bytes.length << 1 | 1);
			this.writeBytes(bytes);
		} else {
			var reference = this.strings.get(string);
			this.writeInteger(reference << 1 | 0);
		}
	} else {
		this.writeInteger(0 << 1 | 1);
	}
};
...
AMF3.Writer.prototype.writeArray = function(array) {
	if (!this.objects.has(array)) {
		...
		this.writeInteger(array.length << 1 | 1);
		// associative part
...
AMF3.Writer.prototype.writeObject = function(object) {
	if (!this.objects.has(object)) {
		...
		if (!this.traits.has(name)) {
			...
			this.writeInteger(properties.length << 4 | dynamic << 3 | externalizable << 2 | 0x03);
			...
			this.writeInteger(reference << 2 | 0x01);
		}
		if (!externalizable) {
...
AMF3.Reader.prototype.readInteger = function() {
	var b0 = this.readByte();
	if (!(b0 & 0x80)) return b0;
	var b1 = this.readByte();
	if (!(b1 & 0x80)) return ((b0 & 0x7F) << 14) | ((b1 & 0x7F) << 7);
	var b2 = this.readByte();
	if (!(b2 & 0x80)) return ((b0 & 0x7F) << 21) | ((b1 & 0x7F) << 14) | ((b2 & 0x7F) << 7);
...
AMF3.Reader.prototype.readObject = function() {
	...
	if (index & 1 << 0) {
		...
		if (!object["@externalizable"]) {
			...
			// deexternalize
			assert(!object["@properties"].length);
